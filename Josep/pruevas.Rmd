...
```
El año que se estrenaron más películas fue el 2000 en que se estrenaron 25. En un 50% de los años se estrenaron como mucho 2 pelis. Tener en cuenta que esto es sobre una muestra de un `r 100*250/17770`% del total de las películas de netflix. 
...
pl <- ggplot(top5, aes(x = Score, y = MovieID, fill = factor(MovieID)))
pl + geom_boxplot() + theme_bw() + coord_flip() + labs(fill = "Year", title = 'Boxplot de la puntuación por película y año de estreno')+
  ...
  # geom_point(mapping=sample[,c(1,3)], col='red', pch=15)

ggplot(top5, aes(x=MovieID, y=Score)) +
...
# barplot(height = movies_onfire$votes, ylim =c(1,80000)) #Cambiando los limites de y, vemos que intervalos son significativos

secuencia <- cut(movies_onfire$votes,
                 breaks = c(min(movies_onfire$votes), 1000, 2000, 3000, 5000, 7000, 10000, 15000,
                            20000, 25000, 30000,35000,40000,45000,55000,max(movies_onfire$votes)),
                 labels=c('0<','1k-2k', '2k-3k', '3k-5k', '5k-7k', '7k-10k', '10k-15k', '15k-20k',
                          '20k-25k', '25k-30k', '30k-35k', '35k-40k', '40k-45k', '45k-55k','>85k'),
...
ggplot(movies_onfire, aes(text = paste('Votes:', votes), y = MovieID, x = Year ), ylab('Top10')) +  #text para visualizar votos en el interactivo
