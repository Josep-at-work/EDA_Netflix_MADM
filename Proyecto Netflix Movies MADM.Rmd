...
Este documento presenta los resultados del análisis exploratorio realizado sobre una muestra aleatoria de las puntuaciones de 250 películas de *Netflix*. El AED explora la distribución de las puntuaciones por distintos períodos de tiempo y compara los estadísticos de las películas que han recibido más votaciones. 
...
n_scores_weekend_weekday_ratio = n_scores_weekend / n_scores 
#el 18% de las valoraciones son en fin de semana, que es menos que el 28% de días que son fin de semana
```
...
ggplot(data = num_users ,aes(Count, Percent)) + 
  geom_bar(stat='identity', colour="black", fill="lightblue")+
  labs(y='%',x='Número de usuarios', title='%Usuarios correspondiente al número de votaciones')
```
