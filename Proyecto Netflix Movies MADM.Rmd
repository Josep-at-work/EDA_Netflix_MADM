...
```{r}
sample = head(movie_score_avg[order(movie_score_avg$n, decreasing = TRUE),],5)
sample
```
###Histograma
```{r, para n}
...
###Plot
```{r}
#Si damos como argumentos la variable Release_Year y la variable lMean_Score, obtenemos una tabla de contingencia, que asignaremos al objeto tab_sample.
tab_sample <- table(sample$Release_Year, sample$Mean_Score)
prop.table(tab_sample, margin = 1)
```

```{r}
plot(x = sample$Release_Year, y = sample$n, col = 'green', 
     main = "test", xlab = "n", ylab = "Release_Year")
...
```{r, para Release_Year}
plot(sample$Release_Year, sample$Mean_Score)
```
